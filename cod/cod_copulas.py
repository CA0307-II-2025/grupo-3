...
def resumen_disbursements(df_filtrado, tipo_col="School Type"):
    ...
    for col in cols_disb:
        for tipo in tipos:
            ...
            mejor_log = (abs(s_log) < abs(s_orig)) and (
                abs(k_log - 3) < abs(k_orig - 3)
            )
...
def diagnosticar_pit(df, col_name):
    ...
    axes[1].plot(
        u_sorted, cdf_teo, label="CDF teórica U(0,1)", color="red", linestyle="--"
    )
    ...
    fig.savefig(
        f"../figs/diagnostico_pit_{col_name.replace(' ', '_').replace('$', '').replace('/', '-')}.png",
        dpi=300,
    )
    plt.show()
...
def plot_copula_contour(model, params, outpng, u_fix=0.5, dims=(0, 1), grid=200):
    ...
    plt.colorbar(cs, shrink=0.85, label="c(u)")
...
# ---------- orquestador ----------
def generate_copula_report(
    df_public, df_private, df_proprietary, outdir="results/copulas", q_extreme=0.95
):
    ensure_dir(outdir)
    ...
    for col in pit_cols:
        ...
        # Carpeta
        var_slug = slug(col)
        vdir = os.path.join(outdir, var_slug)
        ensure_dir(vdir)
            ...
            [
                {
                    ...
                    "AIC": aic,
                    "BIC": bic,
                    "rho12": params["rho"][0, 1],
                    "rho13": params["rho"][0, 2],
                    "rho23": params["rho"][1, 2],
        ...
        tableB = pd.DataFrame(
            [
                {
                    "Par": "Public–Private",
                    "tau_copula": tau_mod.loc["Public", "Private"],
                    "rho_model": params["rho"][0, 1],
                    "lambda_L": lam12,
                    "lambda_U": lam12,
                },
                {
                    "Par": "Public–Proprietary",
                    "tau_copula": tau_mod.loc["Public", "Proprietary"],
                    "rho_model": params["rho"][0, 2],
                    "lambda_L": lam13,
                    "lambda_U": lam13,
                },
                {
                    "Par": "Private–Proprietary",
                    "tau_copula": tau_mod.loc["Private", "Proprietary"],
                    "rho_model": params["rho"][1, 2],
                    "lambda_L": lam23,
                    "lambda_U": lam23,
                },
                {
        ...
        dims_map = {
            ("Public", "Private"): (0, 1),
            ("Public", "Proprietary"): (0, 2),
            ("Private", "Proprietary"): (1, 2),
        }
        ...
        plot_copula_contour(
            winner,
            params,
            os.path.join(vdir, "fig3_contour_top_pair.png"),
            u_fix=0.5,
            dims=dims,
            grid=120,
        )
